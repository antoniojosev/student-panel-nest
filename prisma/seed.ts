...
async function main() {
  ...
  // Students
  const statuses = [StudentStatus.activo, StudentStatus.inactivo, StudentStatus.completado, StudentStatus.abandonado];
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    let progress: number;
    if (status === StudentStatus.completado) progress = 100;
    else if (status === StudentStatus.abandonado) progress = Math.floor(Math.random() * 40);
    else if (status === StudentStatus.inactivo) progress = Math.floor(Math.random() * 60);
